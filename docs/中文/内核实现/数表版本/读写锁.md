...
```c
uint8_t read_acquire(rwlock_handle rw)
{
    // 1. 保护计数器
    if(!semaphore_take(rw->C_guard, 1)){
        return false;
    }
    rw->active_reader++;
...
```c
uint8_t read_release(rwlock_handle rw)
{
    if(!semaphore_take(rw->C_guard, 1)){
        return false;
    }
...
```c
uint8_t write_acquire(rwlock_handle rw)
{
    // 1. 保护计数器
    if(!semaphore_take(rw->C_guard, 1)){
        return false;
    ...
    // 3. 阻塞/放行自己
    if(!semaphore_take(rw->write, 1)){
        return false;
    }

    // 4. 进入写者临界区互斥
    if(!semaphore_take(rw->W_guard, 1)){
        return false;
    }
    return true;
}
...
```c
uint8_t write_release(rwlock_handle rw)
{
    // 1. 退出写者临界区
